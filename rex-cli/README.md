...
Экземпляр класса **ErrorHandler** из библиотеки _errh_ передаётся функциям в аргументе **task.error**. Экземпляр класса **REXStat** из библиотеки _rex-stat_ передаётся в аргументе **task.stat**. Об использовании этих классов см. ниже в разделах про логи и статистику, а также в разделе про API.

cmp.js
------

Дополнительно здесь присутствует утилита cmp.js, предназначенная для тестирования конвертеров. Она принимает на вход файл в формате REX, экспортирует его, дальше импортирует назад и сравнивает полученные данные с оригинальными.

Синтаксис: cmp.js <название формата> <путь к файлу с данными в формате REX> [-s|-v] [-x путь к файлу с исключениями] [-d путь к файлу со словарями]

Ключи командной строки:

 - -s     - вывести статистику по несовпадающим полям
 - -v     - вывести информацию по всем несовпадающим полям каждого объекта (выводит МНОГО информации, если на вход подать достаточное количество объектов)
 - -d     - путь к файлу со словарями, будет передан в конвертер в task.config.dicfile
 - -x     - путь к файлу, хранящему список полей, которые нужно игнорировать при сравнении. Если поле начинается с /, дальнейшее выражение интерпретируется как регулярное
 - -n     - задаёт количество объектов, которые нужно обрабатывать, если не заданы ключи -v или -s (по умолчанию - 5)
