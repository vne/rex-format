...
Лучше считать, что всё, что пишется с помощью console.log (и console.error и т.д.) никто никогда не увидит. То есть,
console.log можно использовать только для отладки, но не для информирования кого-то о чём-то. Для ведения логов в каждую функцию преобразования передаётся объект log (вообще, об аргументах и их смысле см. ниже разделы про API):
...
То есть, логгер можно вызывать как функцию (это соответствует уровню info), а можно вызывать один из его методов:
 - error - ошибка
...
Нужно отметить, что этот логгер предназначен именно для технической информации. Скажем, не удаётся загрузить модуль, не удаётся прочитать файл, то-то сделано, то-то сейчас будет делаться или сделано на столько-то процентов. Информация об ошибках во входных данных в эти логи не пишется, а сохраняется в отдельном массиве в виде, пригодном для отсылки необученному риэлтору. Об этом - ниже в разделе про функции import и export.

В случае, если ошибка настолько серьёзна, что дальнейшее выполнение функции не имеет смысла, функция может досрочно вызывать callback-функцию с описанием ошибки в первом аргументе и достойно умереть. Логи нужны для прояснения ситуации.

Функции import, export и validate - асинхронные, результаты своей деятельности они возвращают через callback-функции. Поэтому они не должны бросать исключений. Для передачи наверх сообщения об ошибке, из-за которой невозможно продолжать работу, им следует использовать первый аргумент callback-функции. Если работа заканчивается штатно, то первый аргумент должен быть null (или undefined).
...
Callback-функция принимает два аргумента: объект ошибки и объект с данными. Если объект ошибки определён, то это означает, что в процессе загрузки данных произошло нечто, из-за чего дальнейшая работа невозможна. Ошибка записывается в лог и программа останавливается.
...
Callback-функция принимает два аргумента: объект ошибки и объект, описывающий результат выполнения.

### Функции import и export

Функции import и export передают в callback-функцию два аргумента: объект ошибки и структуру данных, описывающую результат выполнения:
...
Предполагается, что объект ошибки определён только в том случае, когда происходит какая-то техническая ошибка, не связанная со входными данными. Если ошибка происходит из-за данных, то объект ошибки в первом аргументе callbac-функции должен быть неопределён, а во втором аргументе следует вернуть null в data и описание ошибки (или ошибок) в массиве errors. Если данные можно преобразовать частично (скажем, часть объектов описаны корректно, часть - нет), то в data должны быть преобразованные данные, в errors - описание ошибок.
...
После окончания валидации вызывается callback-функция с двумя аргументами: объект ошибки и результаты работы. Объект ошибки определён только в том случае, если произошла ошибка, не связанная со входными данными.
...
На данный момент, действия могут быть следующие:
 ...
 - archive - добавить файл в архив. Имя архива берётся из параметра file, тип - из параметра type. Тип пока что может быть только один: zip.
